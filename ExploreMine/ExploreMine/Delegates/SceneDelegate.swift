...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if  (UserDefaults.standard.value(forKey: kTutorialViewed) == nil) {
            let mainStoryBoard = UIStoryboard(name: "Main", bundle: nil)
            let ViewController = mainStoryBoard.instantiateViewController(withIdentifier: "tutorialNavigation")
            self.window!.rootViewController = ViewController
        } else if  UserDefaults.standard.value(forKey: kSessionId) == nil{
            let mainStoryBoard = UIStoryboard(name: "Main", bundle: nil)
